...
class KeywordGenerator {
	...
	start() {
		...
		KeywordGenerator.tagfile = new TagFile();
	...
	addKeywordsFromPath(fileDuple, resolve) {
		...
		if (filePath.length <= 5) {
			KeywordGenerator.processedFiles.push(fileDuple.ID);
			KeywordGenerator.filesWithErrors.push({file: fileDuple, error: 'File path too short'});
			resolve();
			return false;
		}

		//NEW WAY
		KeywordGenerator.tagfile.exiftag(fileDuple.dest, fileKeywords, function (success) {
			if (success) {
				KeywordGenerator.processedFiles.push(fileDuple.ID);
			} else {
				KeywordGenerator.filesWithErrors.push({file: fileDuple, error: 'Exif write failed'});
			}
