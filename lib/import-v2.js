...
class ImportFiles {
	...
	getUSBDevices() {
		...
		return new Promise(function (resolve, reject) {
			...
			var ignoredDevices = self.settings.local.importFilter.ignoreDevices.join('|');
			...
			blockutils.getBlockInfo({"ignoredev":"^("+ ignoredDevices +")"}, function (err,json){
				...
				json.forEach(function (el) {
					...
					} else if (el.MOUNTPOINT !== '') {
						ImportFiles.devicePaths.push(el.MOUNTPOINT);
					} else {
						//potentially mount usb stick
					}
				});
	...
	prepareSourceDestinationMapping(_fileListPath) {
		return new Promise(function (resolve, reject) {
			if (ImportFiles.isLocalImport) {
				var ignorepath = ImportFiles.currentInstance.settings.global.filesystem.filePath.src;
			} else {
				//USB import
	...
	importFile(fileObject, callback) {
		...
		//return new Promise(function(resolve, reject){
			...
			if (ImportFiles.runTagger && fileObject.destination) {
				...
				var fileDest = self.settings.global.filesystem.filePath.media.final + fileName;
			} else {
				var fileDest = self.settings.global.filesystem.filePath.media.dest + fileName;
			}
