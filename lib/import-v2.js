...
class ImportFiles {
	...
	start(usbdevice,filepath) {
		...
		ImportFiles.tagfile = new ExifTag();
	...
	processMappedPaths(mappedFiles) {
		...
		if (self.limitFileImportTo > 0) {
			mappedFiles = mappedFiles.splice(0,self.limitFileImportTo);
		}
		...
		self.copyAll(mappedFiles).then(function (results) {
	...
	importFile(fileObject) {
		...
		return new Promise(function(resolve, reject){
			...
			if (fileObject.destination) {
				var uniqueFileName = fileObject.destination;//self.createUniqueFileName(filePath);
			} else {
			...
			if (ImportFiles.runTagger && fileObject.destination) {
				...
				var fileDest = self.settings.filePath.media.final + uniqueFileName;
			} else {
			...
			writeStream.once('finish', function() {
				...
				if (ImportFiles.runTagger && fileObject.keywords) {
					var keywordsArray = self.objectToArray(fileObject.keywords);
					if (keywordsArray.length > 0) {
						ImportFiles.tagfile.exiftag(fileDest, keywordsArray, function (success) {
							if (!success) {
								self.importErrors.push({
										filename: fileDest, 
