...
class ImportFiles {
	...
	start() {
		...
		ImportFiles.db = new sqlite3.Database('./data/photo-station.db');
	...
	processFileList(device,fileList) {
		...
		this.getImportID(device,filesNo, function proceedWithImport(importID) {
			...
			//FOR TESTING
			fileListArr = fileListArr.splice(0,50);
	...
	copyComplete(self) {
			...
			self.importedFiles.forEach(function (file) {
				ImportFiles.db.run("INSERT INTO imported_files (importID,src,dest,status) VALUES ($importid,$src,$dest,$status)", {
					...
					if (err) {
						...
						self.importErrors.push({filenam:'',error:"Can't insert file import: " + file.src + " DEST: " + file.dest});
					}
				});
			});
		});
		...
		ImportFiles.db.serialize(function () {
			...
			ImportFiles.db.run("UPDATE imports SET end = $end, filesOut = $files, status = $status WHERE ID = $importid", {
				$importid: ImportFiles.currentImportID,
				$end: moment().format(),
				...
				if (err) {
					...
					self.importErrors.push({filenam:'',error:"Can't update database import enddate and filecount"});
				}
			});
		});
